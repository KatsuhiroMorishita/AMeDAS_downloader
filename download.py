...
# created: 2015-08-17
# history: 2023-10-18   dailyでの保存の場合に日付を月初めにするように変更。字下げ文字を半角スペースに置換した。
#                       気象庁HPの仕様変更でreal-timeは取得できなくなっている。未対応。
#                       APIを使った方が絶対によい…。
...
class amedas_node:
    ...
    def save(self, _type="10min", date=None, force=False):
        ...
        date_copy = date
        if _type == "daily":
        	date_copy = dt(year=date.year, month=date.month, day=1)

        # 保存するファイル名とパスを作製
        fname = self._block_no + "_" + self.name + date_copy.strftime("_%Y_%m_%d") + ".html"
        path = os.path.join(_dir, fname)

        # ダウンロード済みでない、またはウンロードが指示されている場合はダウンロードを試みる
        if os.path.exists(path) == False or force == True: 
            html = self.get_data(_type, date_copy)
            if html != None:
